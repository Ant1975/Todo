...
bool WebServer::dispatchCommand(ConnectionType requestType, char *verb,
	...
	if (verb[0] == '/') {
		...
		for (i = 0; i < this->bindingsLenght; ++i) {
			if ((verb_len == strlen(bindings[i].uri))
					&& (strncmp(verb, bindings[i].uri, verb_len) == 0)) {
				// Skip over the "verb" part of the URL (and the question
				// mark, if present) when passing it to the "action" routine

				processRestReguest(requestType, verb + verb_len + qm_offset,
						bindings[i]);
				return true;
			}
		}
...
void WebServer::defaultFailCmd(WebServer &server,
		WebServer::ConnectionType type, char *url_tail, bool tail_complete) {
	server.httpFail();
}
...
bool WebServer::readPOSTparam(char *name, int nameLen, char *value,
		int valueLen) {
...
void WebServer::getRequest(WebServer::ConnectionType &type, char *request,
		int *length) {
...
void WebServer::processRestReguest(ConnectionType type, char *tail,
		Binding & binding) {
	...
	if (type == WebServer::GET) {
		...
		if (!id) {
			...
			aJson.print(*binding.jsonObject, *this);
		}

		else {

			aJsonObject *child = (*binding.jsonObject)->child;

			while (child) {

				aJsonObject* idItem = aJson.getObjectItem(child, "id");

				if (idItem->valueint == id) {
					httpSuccess("application/json");
					aJson.print(child, *this);
					break;
				}
	...
	if (type == WebServer::POST) {
		...
		aJsonObject * newItem = aJson.parse(*this);
		aJson.addNumberToObject(newItem, "id", ++binding.curId);
		aJson.addItemToArray(*binding.jsonObject, newItem);
		httpCreated("application/json");
		aJson.print(newItem, *this);
	...
	if (type == WebServer::PUT) {
		...
		if (!id) {
			aJson.deleteItem(*binding.jsonObject);
			*binding.jsonObject = aJson.parse(*this);
			httpSuccess();
		} else {
			aJsonObject *child = (*binding.jsonObject)->child;
			...
			while (child) {
				...
				if (idItem->valueint == id) {
					aJson.replaceItemInArray(*binding.jsonObject, i,
							aJson.parse(*this));
					httpSuccess();
					break;
				}
	...
	if (type == WebServer::DELETE) {
		...
		while (child) {
			...
			if (idItem->valueint == id) {
				...
				aJson.deleteItemFromArray(*binding.jsonObject, i);
				httpSuccess();
				break;
			}
			child = child->next;
...
void WebServer::addJSONBinding(aJsonObject ** jsonObject, char * uri) {

	if (bindingsLenght < bindingsMaxLenght) {
		bindings[bindingsLenght].jsonObject = jsonObject;
		...
		while (child) {
			...
			if (idObject->valueint > maxId) {
				maxId = idObject->valueint;
			}
			child = child->next;
		}
